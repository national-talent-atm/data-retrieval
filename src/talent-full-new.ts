...
const inputStream = ReadableStream.from(
  ...
  .pipeThrough(
    (() => {
      ...
      return map((value) => {
        const [id, firstName, lastName, ind, ...rest] = value
          .split('\t')
          .map((term) => term.trim());
        ...
        return { id, name, ind, index, rest: [firstName, lastName, ...rest] };
      });
    })(),
  );
  ...
  .pipeThrough(
    map(([authorBody, metricsBody, scopusSearchBody]) => {
      ...
      return {
        ...
        keyword1: sortedKeywordEntries[0]?.[0] ?? keywords.shift(),
        keyword2: sortedKeywordEntries[1]?.[0] ?? keywords.shift(),
        keyword3: sortedKeywordEntries[2]?.[0] ?? keywords.shift(),
        keyword4: sortedKeywordEntries[3]?.[0] ?? keywords.shift(),
        keyword5: sortedKeywordEntries[4]?.[0] ?? keywords.shift(),
        ...
        co_author6:
          extractName(sortedCoauthorEntries[5]?.[1]?.author) ?? co_author6,
        co_author7:
          extractName(sortedCoauthorEntries[6]?.[1]?.author) ?? co_author7,
        co_author8:
          extractName(sortedCoauthorEntries[7]?.[1]?.author) ?? co_author8,
        co_author9:
          extractName(sortedCoauthorEntries[8]?.[1]?.author) ?? co_author9,
        co_author10:
          extractName(sortedCoauthorEntries[9]?.[1]?.author) ?? co_author10,
      };
    }),
  );
